"""요약 — 스펙 v4 §3.2. 스트림릿은 게이트·헤더·데이터 로딩만 하고,
본문(스트립·히어로 차트·2단·상위 8)은 views.summary가 그린 HTML을 iframe으로 띄운다."""
import os, sys
...
from report_dashboard.views import summary
...
if ctx is None:
    st.markdown(ui.empty_state("등록된 콘텐츠가 없습니다", "콘텐츠가 등록되고 첫 수집이 끝나면 숫자가 채워집니다."), unsafe_allow_html=True); st.stop()
frame.render(summary.build(ctx, campaign))
